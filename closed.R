...
{
	x=0:100/100; t=2*pi*x
	png('temp.png',height=100,width=100);par(mar=c(0,0,0,0))
	z=shape(t)
	plot(0, xlim=c(-5,5), ylim=c(-5,5), ty='n', axes=FALSE, asp=1, xlab='', ylab='')
	polygon(z*cos(t),z*sin(t), col=1)
	dev.off()
	# png(paste0('output.',i,'.png'),height=100,width=100);par(mar=c(0,0,0,0))
	...
	Z=v
	Z[which(!z, arr.ind = TRUE)]=w[which(!z, arr.ind = TRUE)]
	Z=t(Z[nrow(Z):1,])
